...
// ─── Reliable Cancel / Force-Abort (cancel-teardown feature) Field Tests ─────────

// TestDelegationCancelPayload_RequestIDRoundtrip validates the additive
// idempotency request_id on the cancel frame: a non-empty request_id appears on
// the wire and survives round-trip, while an empty request_id is omitted
// (omitempty) so a pre-feature producer marshals to byte-identical JSON.
